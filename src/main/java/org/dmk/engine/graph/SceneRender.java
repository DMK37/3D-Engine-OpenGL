...
public class SceneRender {
    ...
    private void changeShaders(ShadingType shadingType) {
        shaderProgram.cleanup();
        List<ShaderProgram.ShaderModuleData> shaderModuleDataList = new ArrayList<>();
        switch (shadingType) {
            case FLAT:
                shaderModuleDataList.add(new ShaderProgram.ShaderModuleData("resources/shaders/scene_f.vert", GL_VERTEX_SHADER));
                shaderModuleDataList.add(new ShaderProgram.ShaderModuleData("resources/shaders/scene_f.frag", GL_FRAGMENT_SHADER));
                break;
            case GOURAUD:
                shaderModuleDataList.add(new ShaderProgram.ShaderModuleData("resources/shaders/scene_g.vert", GL_VERTEX_SHADER));
                shaderModuleDataList.add(new ShaderProgram.ShaderModuleData("resources/shaders/scene_g.frag", GL_FRAGMENT_SHADER));
                break;
            case PHONG:
                shaderModuleDataList.add(new ShaderProgram.ShaderModuleData("resources/shaders/scene.vert", GL_VERTEX_SHADER));
                shaderModuleDataList.add(new ShaderProgram.ShaderModuleData("resources/shaders/scene.frag", GL_FRAGMENT_SHADER));
    ...
    public void render(Scene scene) {
        if(shadingType != scene.getShadingType()) {
            shadingType = scene.getShadingType();
            changeShaders(shadingType);
        }
        glEnable(GL_BLEND);
...
//            Vector4f aux = new Vector4f();
//            aux.set(coneDirection, 1);
//            aux.mul(viewMatrix);
//            coneDirection.set(aux.x, aux.y, aux.z);
